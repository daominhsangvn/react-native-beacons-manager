...
function startMonitoringForRegion(region: BeaconRegion): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    BeaconsManager.startMonitoring(
      region.identifier,
      region.uuid,
      region.minor ? parseInt(region.minor) : -1,
      region.major ? parseInt(region.major) : -1,
      resolve,
      reject,
    );
  });
...
/**
 * Start or stop all beacon services
 *
 * @param {BeaconRegion} regionId if string or region: BeaconRegion object
 * @returns {Promise<any>} promise: resolves to void when successful
 */
function startBeaconServices(region: BeaconRegion): Promise<any> {
  return new Promise((resolve, reject) => {
    BeaconsManager.startBeaconServices(
      region.identifier,
      region.uuid,
      region.minor ? parseInt(region.minor) : -1,
      region.major ? parseInt(region.major) : -1,
...
function stopBeaconServices(region: BeaconRegion): Promise<any> {
  return new Promise((resolve, reject) => {
    BeaconsManager.stopBeaconServices(
      region.identifier,
      region.uuid,
      region.minor ? parseInt(region.minor) : -1,
      region.major ? parseInt(region.major) : -1,
...
function isStarted(): Promise<boolean> {
  return new Promise((resolve) => {
    BeaconsManager.isStarted(
      resolve
    );
  });
}
